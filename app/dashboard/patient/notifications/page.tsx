...
export default function PatientNotifications() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setStats(s => ({ ...s, rendezvous: r.data.length, consultations: c.data.length, ordonnances: o.data.length }));
    }).catch(() => {});
  ...
  const markAllRead = async () => {
    ...
    await Promise.all(unread.map(n => api.patch(`notifications/${n.id}/`, { lu: true }).catch(() => {})));
  ...
  return (
    <PrivateRoute allowedRoles={["patient"]}>
      <style>{`
        ...
        .stat-icon { width:40px; height:40px; border-radius:12px; background:linear-gradient(135deg,#8B5CF6,#10B981); display:flex; align-items:center; justify-content:center; font-size:18px; flex-shrink:0; box-shadow:0 4px 10px rgba(139,92,246,.25); }
        ...
        .mark-btn { padding:6px 12px; border:1.5px solid rgba(139,92,246,.2); border-radius:8px; background:rgba(139,92,246,.05); font-family:'DM Sans',sans-serif; font-size:11px; font-weight:700; color:#8B5CF6; cursor:pointer; transition:all .2s; white-space:nowrap; flex-shrink:0; }
          ...
          <div className="notif-list">
            {loading ? (
              Array.from({ length: 4 }).map((_, i) => (
                <div key={i} style={{ background: "white", borderRadius: 18, padding: "18px 22px", display: "flex", gap: 16, border: "1.5px solid #F1F5F9" }}>

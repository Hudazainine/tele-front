...
export default function PatientProfil() {
  ...
  const [form, setForm]       = useState<FormData>({ username: "", email: "", allergies: "", historique: "" });
  const [saving, setSaving]   = useState(false);
  const [toast, setToast]     = useState<{ msg: string; ok: boolean } | null>(null);
  const [stats, setStats]     = useState({ rendezvous: 0, consultations: 0, ordonnances: 0 });
  ...
  useEffect(() => {
    ...
    api.get("users/me/").then((r) => {
      setProfil(r.data);
      setForm({ username: r.data.username || "", email: r.data.email || "", allergies: r.data.allergies || "", historique: r.data.historique || "" });
    }).catch(() => {});
    Promise.all([api.get("rendezvous/"), api.get("consultations/"), api.get("ordonnances/")])
      .then(([r, c, o]) => setStats({ rendezvous: r.data.length, consultations: c.data.length, ordonnances: o.data.length }))
      .catch(() => {});
  ...
  const cancelEdit = () => {
    if (profil) setForm({ username: profil.username || "", email: profil.email || "", allergies: profil.allergies || "", historique: profil.historique || "" });
    setEdit(false);
  };
    ...
    <PrivateRoute allowedRoles={["patient"]}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .root { min-height:100vh; background:#F4F2F9; background-image:radial-gradient(at 5% 5%,rgba(139,92,246,.08) 0,transparent 50%),radial-gradient(at 95% 95%,rgba(16,185,129,.07) 0,transparent 50%); font-family:'Instrument Sans',sans-serif; display:flex; }
        ...
        .id-banner { height:100px; background:linear-gradient(135deg,#8B5CF6 0%,#10B981 100%); position:relative; overflow:hidden; }
        ...
        .field-input:focus { border-color:#8B5CF6; background:#fff; box-shadow:0 0 0 3px rgba(139,92,246,.1); }
          ...
          <div className="layout">
            ...
            <div className="id-card">
              ...
              <div className="id-body">
                ...
                <div><span className="id-tag">🩺 Patient</span></div>
                ...
                <div className="id-info">
                  ...
                  <div className="id-row">
                    ...
                    <div>
                      <div style={{ fontSize: 10, color: "#94A3B8", fontWeight: 600, textTransform: "uppercase", letterSpacing: ".5px" }}>Allergies</div>
                      <div className="id-val" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                  ...
                  <div className="id-row">
                    ...
                    <div style={{ display: "flex", gap: 14 }}>
                      {[{ v: stats.rendezvous, l: "RDV" }, { v: stats.consultations, l: "Consult." }, { v: stats.ordonnances, l: "Ordonn." }].map(s => (
                        <div key={s.l} style={{ textAlign: "center" }}>
                          <div style={{ fontFamily: "'Bricolage Grotesque',sans-serif", fontSize: 18, fontWeight: 800, color: "#7C3AED" }}>{s.v}</div>
                  ...
                  <div className="fields-grid">
                    <div className="field-wrap">
                      ...
                      {edit
                        ...
                        : <div className="field-val">{profil.username || <span className="field-empty">Non renseigné</span>}</div>}
                    </div>
                    <div className="field-wrap">
                      ...
                      {edit
                        ...
                        : <div className="field-val">{profil.email || <span className="field-empty">Non renseigné</span>}</div>}
                    </div>
                    <div className="field-wrap full">
                      <div className="field-label">Allergies</div>
                      {edit
                        ? <textarea className="field-input field-textarea" value={form.allergies} placeholder="Décrivez vos allergies connues…" onChange={e => setForm(p => ({ ...p, allergies: e.target.value }))} />
                        : <div className="field-val" style={{ minHeight: 60 }}>{profil.allergies || <span className="field-empty">Aucune allergie renseignée</span>}</div>}
                    </div>
                    <div className="field-wrap full">
                      <div className="field-label">Historique médical</div>
                      {edit
                        ? <textarea className="field-input field-textarea" style={{ minHeight: 110 }} value={form.historique} placeholder="Antécédents médicaux, chirurgies, traitements…" onChange={e => setForm(p => ({ ...p, historique: e.target.value }))} />
                        : <div className="field-val" style={{ minHeight: 80 }}>{profil.historique || <span className="field-empty">Aucun historique renseigné</span>}</div>}
                    </div>
                  </div>
                </div>
                ...
                <div className="panel-body">
                  <div className="sec-row">
                    <div>
                      ...
                      <div className="sec-desc">Protège l'accès à votre espace santé</div>
                    </div>
                  ...
                  <div className="sec-row">
                    <div>
                      ...
                      <div className="sec-desc">{profil.email || "Aucun email configuré"}</div>
                    </div>
                  ...
                  <div className="sec-row">
                    <div>
                      ...
                      <div className="sec-desc">Authentification JWT · Expiration 24h</div>
                    </div>
